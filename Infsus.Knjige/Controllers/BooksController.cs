...
public class BooksController : Controller
{
    ...
    public IActionResult Create()
    {
        var authors = _context.Authors
            .Select(a => new SelectListItem { Value = a.AuthorId.ToString(), Text = $"{a.Name} {a.Surname}" })
            .ToList();
        
        var genres = _context.Genres
            .Select(g => new SelectListItem { Value = g.GenreId.ToString(), Text = g.GenreName })
            .ToList();

        var vm = new CreateBookViewModel { Authors = authors, Genres = genres };
        return View(vm);
    }
    ...
    {
        var book = (await _mediator.Send(new GetBooksQuery())).Find(b => b.BookId == id);
        if (book == null)
        {
            return NotFound();
        }
